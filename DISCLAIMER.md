...
This project is an independent reference implementation informed by professional experience with institutional RAG systems. Its purpose is to explain engineering decisions through a public portfolio, not to reproduce an employer's production application.
...
There is no standalone license file. Earlier documentation used an MIT label, but the showcase update does not infer a complete rights grant from that label. The owner must confirm the provenance of existing material and the intended license before a license file is added.

See the [feature matrix](docs/FEATURE_STATUS.md), [limitations](docs/LIMITATIONS.md), and [security policy](SECURITY.md) for the current review boundaries.
