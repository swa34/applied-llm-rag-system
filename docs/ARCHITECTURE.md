...
`demo/cli.mjs` provides interactive and one-question commands; the scenario runner exercises only the development manifest. The versioned corpus manifest allowlists ten documents and assigns 38 stable passage IDs. The separate held-out manifest was run only through the frozen evaluation path after its baseline commit; the [Phase 6 report](../evaluation/fictional/v1/results/phase-6/README.md) preserves six raw runs and all-turn review. Ten focused live checks on the earlier four-document corpus passed on 2026-09-12; see [LOCAL_DEMO.md](LOCAL_DEMO.md#verification-record). There is no answer cache, persistent index, streaming server, or browser interface in this slice. See [LOCAL_DEMO.md](LOCAL_DEMO.md) for commands, diagnostics, and provider tradeoffs.
